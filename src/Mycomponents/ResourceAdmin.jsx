...
export default function ProjectNAvAdmin() {
    ...
    let video1 = '';
    // console.log(pdf1)
    const imageRefresource = ref(storage, `imageresource/${`${image}` + uuidv4()}`)
    const videoRef = ref(storage, `video/${'video' + uuidv4()}`)
    const pdfRefresource = ref(storage, `pdfresource/${`${pdf}` + uuidv4()}`)
    let id = []
    let data
    let data1
    useEffect(() => {
    ...
    const updateProfile = async () => {
        ...
        await uploadBytes(pdfRefresource, pdf).then((res) => {
            try {
        ...
        await uploadBytes(imageRefresource, image).then((res) => {
            try {
                                ...
                                <Card.Body>
                                    ...
                                    <div className='ResourceCardSection'>
                                        ...
                                        <div className='readmore'>
                                            <Link to={values.pdf} style={{ color: "#0085FF" }}>
                                                <p>Read More </p>
                                            </Link>
